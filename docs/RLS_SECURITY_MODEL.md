...
`get_assignment_analytics(assignment_id)` uses the same internal owner/admin/educator authorization boundary and active-workspace requirement. It is executable only by `authenticated`; `anon` and `PUBLIC` have no execute privilege. Students, outsiders, and foreign-workspace staff receive `not_authorized` even when they know an assignment UUID. The function aggregates only the target assignment's items/results and active class enrollments. It returns the enrolled students' minimal email/progress/performance rows only to the authorized staff caller, never exposes `auth.users` generally, and does not grant direct teacher SELECT on the result table. The aggregate is read-only and does not add an index or stored summary.
...
The tests prove PostgreSQL grants/RLS behavior for the local migrations. Phase 6 additionally runs an isolated local Auth/Data API smoke using ordinary authenticated teacher/student clients for workspace bootstrap, assignment publication, class joining, result recording, and analytics reads. The smoke uses a local service credential only in its non-browser test harness to provision and clean up temporary local users. This does not certify or change the unbaselined hosted Calcura project.
